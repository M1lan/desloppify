...
class TestKotlinZoneRules:
    ...
    def test_production_source(self):
        assert (
            classify_file("src/main/kotlin/com/app/Feature.kt", KOTLIN_ZONE_RULES)
            == Zone.PRODUCTION
        )
    ...
    def test_test_suffix_test_kt(self):
        assert (
            classify_file("src/test/kotlin/com/app/FeatureTest.kt", KOTLIN_ZONE_RULES)
            == Zone.TEST
        )

    def test_test_suffix_spec_kt(self):
        assert (
            classify_file("src/test/kotlin/com/app/FeatureSpec.kt", KOTLIN_ZONE_RULES)
            == Zone.TEST
        )

    def test_test_src_test_dir(self):
        assert (
            classify_file("src/test/kotlin/com/app/Helper.kt", KOTLIN_ZONE_RULES)
            == Zone.TEST
        )

    def test_test_android_test_dir(self):
        assert (
            classify_file("src/androidTest/kotlin/com/app/UITest.kt", KOTLIN_ZONE_RULES)
            == Zone.TEST
        )

    def test_test_common_test_dir(self):
        assert (
            classify_file("src/commonTest/kotlin/TestUtils.kt", KOTLIN_ZONE_RULES)
            == Zone.TEST
        )

    def test_test_jvm_test_dir(self):
        assert (
            classify_file("src/jvmTest/kotlin/JvmTest.kt", KOTLIN_ZONE_RULES)
            == Zone.TEST
        )

    def test_test_ios_test_dir(self):
        assert (
            classify_file("src/iosTest/kotlin/IosTest.kt", KOTLIN_ZONE_RULES)
            == Zone.TEST
        )
    ...
    def test_generated_build_dir(self):
        assert (
            classify_file(
                "build/generated/source/kapt/main/Module.kt", KOTLIN_ZONE_RULES
            )
            == Zone.GENERATED
        )
    ...
    def test_generated_annotation(self):
        assert (
            classify_file("com/app/Foo.generated.kt", KOTLIN_ZONE_RULES)
            == Zone.GENERATED
        )

    def test_generated_dir(self):
        assert (
            classify_file("generated/com/app/Foo.kt", KOTLIN_ZONE_RULES)
            == Zone.GENERATED
        )
    ...
    def test_script_buildsrc(self):
        assert (
            classify_file("buildSrc/src/main/kotlin/Dependencies.kt", KOTLIN_ZONE_RULES)
            == Zone.SCRIPT
        )
    ...
    def test_common_test_dir(self):
        ...
        assert (
            classify_file("tests/integration/SomeTest.kt", KOTLIN_ZONE_RULES)
            == Zone.TEST
        )
