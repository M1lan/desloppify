...
def _add_cluster_subparser(plan_sub) -> None:
    ...
    p_cc.add_argument(
        "--description", type=str, default=None, help="Cluster description"
    )
    p_cc.add_argument(
        "--action",
        type=str,
        default=None,
        help="Primary action/command for this cluster",
    )
    p_cc.add_argument(
        "--priority", type=int, default=None, help="Priority (lower = higher priority)"
    )
    p_cc.add_argument(
        "--steps-file",
        "-f",
    ...
    p_ca.add_argument(
        "--dry-run", action="store_true", default=False, help="Preview without saving"
    )
    ...
    p_cm.add_argument(
        "cluster_names", type=str, help="Cluster name(s), comma-separated for multiple"
    )
    p_cm.add_argument(
        "position",
    ...
    p_cm.add_argument(
        ...
        nargs="?",
        default=None,
        help="Target issue/cluster (before/after) or integer offset (up/down)",
    )
    p_cm.add_argument(
        "--item",
        dest="item_pattern",
    ...
    p_cmerge = cluster_sub.add_parser(
        "merge", help="Merge source cluster into target (moves issues, deletes source)"
    )
    p_cmerge.add_argument(
        "source", type=str, help="Source cluster name (will be deleted)"
    )
    p_cmerge.add_argument(
        "target", type=str, help="Target cluster name (receives issues)"
    )

    # plan cluster update <name> [--description "..."] [structured step flags]
    p_cu = cluster_sub.add_parser(
        "update", help="Update cluster description and/or action steps"
    )
    ...
    p_cexport = cluster_sub.add_parser(
        "export", help="Export cluster steps to editable format"
    )
    ...
    p_cimport = cluster_sub.add_parser(
        "import", help="Bulk create/update clusters from YAML"
    )
    p_cimport.add_argument("file", type=str, help="YAML file path")
    p_cimport.add_argument(
        "--dry-run",
        action="store_true",
        default=False,
