...
def test_py_extract_functions_aggregates_from_discovered_files(monkeypatch) -> None:
    monkeypatch.setattr(
        python_helpers_mod, "find_py_files", lambda _path: ["src/a.py", "src/b.py"]
    )
    monkeypatch.setattr(
        python_helpers_mod,
...
def test_scan_root_from_files_handles_common_missing_and_invalid_sets(tmp_path) -> None:
    assert (
        python_helpers_mod.scan_root_from_files(["README.md", "docs/index.txt"]) is None
    )
    ...
    assert (
        python_helpers_mod.scan_root_from_files(["src/a.py", "/tmp/project/src/b.py"])
        is None
    )
