...
class _FakeNode:
    def __init__(
        self, node_type: str, children: list["_FakeNode"] | None = None
    ) -> None:
        self.type = node_type
...
def test_make_callback_depth_compute_returns_depth_for_nested_closures(
    monkeypatch,
) -> None:
...
def test_make_callback_depth_compute_returns_none_for_missing_or_shallow(
    monkeypatch,
) -> None:
    spec = SimpleNamespace(grammar="javascript")
    ...
    shallow_tree = SimpleNamespace(
        root_node=_FakeNode("program", [_FakeNode("function_expression")])
    )
    monkeypatch.setattr(callbacks_mod, "_ensure_parser", _fake_ensure)
    monkeypatch.setattr(
        callbacks_mod,
...
def test_ensure_parser_initializes_cache_with_optional_query(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        shared_mod, "_make_query", lambda _lang, query: f"query:{query}"
    )
