...
def test_graph_helpers_build_internal_edges_and_builder(
    ...
    matches = {
        str(source_file): [
            (
                0,
                {
                    "path": FakeNode("string", text="'Thing'"),
                    "prefix": FakeNode("identifier", text="App"),
                },
            ),
...
def test_import_normalize_helpers_strip_comments_and_log_lines() -> None:
    ...
    assert (
        normalize_mod.normalize_body(source, func_node, spec)
        == "def sample():\n    keep()"
    )


def test_resolver_cache_reads_module_path_and_reset_clears_cache(
    tmp_path: Path,
) -> None:
    ...
    assert (
        resolver_cache_mod.read_go_module_path(str(tmp_path / "missing.go.mod")) == ""
    )
...
def test_backend_import_resolvers_cover_language_specific_paths(tmp_path: Path) -> None:
    ...
    assert backend_mod.resolve_go_import(
        "example.com/demo/pkg/util", str(tmp_path / "main.go"), str(tmp_path)
    ) == str(go_file)
    assert (
        backend_mod.resolve_go_import("fmt", str(tmp_path / "main.go"), str(tmp_path))
        is None
    )
    ...
    assert backend_mod.resolve_rust_import(
        "crate::nested::Thing", "", str(tmp_path)
    ) == str(rust_mod)
    assert backend_mod.resolve_rust_import("crate::leaf", "", str(tmp_path)) == str(
        rust_leaf
    )
    ...
    assert backend_mod.resolve_java_import("com.acme.App", "", str(tmp_path)) == str(
        java_file
    )
    ...
    assert backend_mod.resolve_kotlin_import(
        "com.acme.Feature", "", str(tmp_path)
    ) == str(kotlin_file)
    ...
    assert backend_mod.resolve_cxx_include(
        "local.h", str(tmp_path / "src" / "main.c"), str(tmp_path)
    ) == str(local_header)
    assert backend_mod.resolve_cxx_include(
        "shared.h", str(tmp_path / "src" / "main.c"), str(tmp_path)
    ) == str(include_file)
    ...
    assert backend_mod.resolve_csharp_import("Demo.Widget", "", str(tmp_path)) == str(
        csharp_file
    )
    ...
    assert backend_mod.resolve_dart_import(
        "package:demo/src/utils.dart", "", str(tmp_path)
    ) == str(dart_file)
    ...
    assert backend_mod.resolve_scala_import("com.acme.Thing", "", str(tmp_path)) == str(
        scala_file
    )
    ...
    assert backend_mod.resolve_swift_import(
        "Core", str(tmp_path / "App.swift"), str(tmp_path)
    ) == str(swift_file)
...
def test_kotlin_import_resolver_kmp_source_sets(tmp_path: Path) -> None:
    common_file = (
        tmp_path / "src" / "commonMain" / "kotlin" / "com" / "acme" / "Common.kt"
    )
    ...
    assert backend_mod.resolve_kotlin_import(
        "com.acme.Common", "", str(tmp_path)
    ) == str(common_file)
    assert backend_mod.resolve_kotlin_import(
        "com.acme.JvmImpl", "", str(tmp_path)
    ) == str(jvm_file)


def test_kotlin_import_resolver_returns_none_for_missing_import(tmp_path: Path) -> None:
    assert (
        backend_mod.resolve_kotlin_import("com.acme.DoesNotExist", "", str(tmp_path))
        is None
    )
...
def test_functional_import_resolvers_cover_common_conventions(tmp_path: Path) -> None:
    ...
    assert functional_mod.resolve_elixir_import(
        "MyApp.Worker", "", str(tmp_path)
    ) == str(elixir_file)
    ...
    assert functional_mod.resolve_zig_import(
        "util", str(zig_dir / "main.zig"), str(tmp_path)
    ) == str(zig_file)
    assert (
        functional_mod.resolve_zig_import(
            "std", str(zig_dir / "main.zig"), str(tmp_path)
        )
        is None
    )
    ...
    assert functional_mod.resolve_haskell_import(
        "Demo.Feature", "", str(tmp_path)
    ) == str(haskell_file)
    ...
    assert functional_mod.resolve_erlang_include(
        "demo.hrl", str(tmp_path / "src" / "demo.erl"), str(tmp_path)
    ) == str(erlang_file)

    ocaml_file = tmp_path / "lib" / "worker.ml"
    ocaml_file.parent.mkdir(parents=True, exist_ok=True)
    ocaml_file.write_text("let run () = ()\n", encoding="utf-8")
    assert functional_mod.resolve_ocaml_import(
        "MyApp.Worker", "", str(tmp_path)
    ) == str(ocaml_file)
    ...
    assert functional_mod.resolve_fsharp_import(
        "Demo.Worker", "", str(tmp_path)
    ) == str(fsharp_file)
...
def test_functional_import_resolver_returns_none_when_umbrella_listing_fails(
    ...
    assert (
        functional_mod.resolve_elixir_import("MyApp.Worker", "", str(tmp_path)) is None
    )


def test_script_import_resolvers_cover_cached_paths_and_extensions(
    tmp_path: Path,
) -> None:
    ruby_file = tmp_path / "lib" / "helper.rb"
    ruby_file.parent.mkdir(parents=True)
    ruby_file.write_text("module Helper\nend\n", encoding="utf-8")
    assert scripts_mod.resolve_ruby_import(
    ...
    assert scripts_mod.resolve_php_import(
        "App\\Models\\User", "", str(tmp_path)
    ) == str(php_file)
    ...
    assert scripts_mod.resolve_js_import(
        "./ui", str(tmp_path / "src" / "main.js"), str(tmp_path)
    ) == str(js_file)

    bash_file = tmp_path / "scripts" / "shared.sh"
    bash_file.parent.mkdir(parents=True)
    bash_file.write_text("echo ok\n", encoding="utf-8")
    assert scripts_mod.resolve_bash_source(
        "scripts/shared", str(tmp_path / "run.sh"), str(tmp_path)
    ) == str(bash_file)

    perl_file = tmp_path / "lib" / "Demo" / "Worker.pm"
    perl_file.parent.mkdir(parents=True)
    perl_file.write_text("package Demo::Worker;\n1;\n", encoding="utf-8")
    assert scripts_mod.resolve_perl_import("Demo::Worker", "", str(tmp_path)) == str(
        perl_file
    )
    ...
    assert scripts_mod.resolve_r_import(
        "helpers.R", str(tmp_path / "analysis.R"), str(tmp_path)
    ) == str(r_file)
