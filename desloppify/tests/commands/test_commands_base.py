"""Tests for desloppify.languages._framework.commands.base — command factory functions.
...
class TestMakeCmdLarge:
    ...
    def test_calls_detect_large_files(self, mock_display):
        ...
        with patch(
            "desloppify.engine.detectors.large.detect_large_files", return_value=([], 0)
        ) as mock_detect:
            cmd(_make_args())
...
class TestMakeCmdComplexity:
    ...
    def test_uses_threshold_override_from_args(self, mock_display):
        ...
        cmd = make_cmd_complexity(
            file_finder=lambda p: [], signals=[], default_threshold=15
        )
    ...
    @patch("desloppify.languages._framework.commands.base.display_entries")
    def test_falls_back_to_default_threshold_when_args_threshold_is_none(
        self, mock_display
    ):
...
class TestMakeCmdFacade:
    ...
    def test_json_output(self, capsys):
        ...
        cmd = make_cmd_facade(
            build_dep_graph_fn=mock_graph, detect_facades_fn=mock_detect
        )
