...
def test_init_template_registers_language_and_defaults():
    ...
    assert (
        "from desloppify.languages._framework.base.phase_builders import (" in init_py
    )
    assert (
    ...
    assert (
        "from desloppify.languages._framework.base.types import LangConfig" in phases_py
    )

    commands_py = files["commands.py"]
    assert "from desloppify.base.output.terminal import colorize" in commands_py
    assert (
        'print(colorize("swift: placeholder detector command (not implemented)", "yellow"))'
        in commands_py
    )
